#!/usr/bin/env python3
from __future__ import annotations

import argparse
from pathlib import Path
import sys

if __package__ in (None, ""):
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from androidlibboxlite.errors import ReleaseError
from androidlibboxlite.lockfile import ReleaseLock
from androidlibboxlite.manifest import write_release_metadata
from androidlibboxlite.verify import VerificationReport


def main() -> int:
    parser = argparse.ArgumentParser(description="生成 AndroidLibBoxLite 构建清单和校验清单")
    parser.add_argument("--lock", type=Path, required=True)
    parser.add_argument("--aar", type=Path, required=True)
    parser.add_argument("--sources", type=Path, required=True)
    parser.add_argument("--verification", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    try:
        lock = ReleaseLock.from_json(args.lock.read_bytes())
        report = VerificationReport.from_json(args.verification.read_bytes())
        manifest, sums = write_release_metadata(lock, report, args.aar, args.sources, args.output)
        print(manifest)
        print(sums)
        return 0
    except (OSError, ReleaseError) as failure:
        print(str(failure), file=sys.stderr)
        return 2


if __name__ == "__main__":
    raise SystemExit(main())
